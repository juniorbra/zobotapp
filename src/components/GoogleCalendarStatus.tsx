...
const GoogleCalendarStatus: React.FC<GoogleCalendarStatusProps> = ({ agentId, onStatusChange }) => {
  ...
  const [settings, setSettings] = useState<CalendarSettings>({
    ...
    auto_delete: false,
    custom_instructions: defaultInstructions
  });
  ...
  useEffect(() => {
    const checkGoogleConnection = async () => {
      ...
      try {
        ...
        if (onStatusChange) onStatusChange(isGoogleConnected);
        if (agentId) loadAgentSettings(agentId);
      } catch (error) {
    ...
    try {
      ...
      if (data) {
        setSettings({
          ...
          custom_instructions: data.google_calendar_custom_instructions || defaultInstructions
        });
      }
    } catch (error) {
      // silent
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newDays = e.target.checked
                        ? [...settings.allowed_days, day]
                        : settings.allowed_days.filter(d => d !== day);
